...
export default function Home() {
  ...
  useEffect(() => {
    // Função assíncrona para obter filmes até 12 anos
    async function fetchMovies() {
      try {
        ...
        const response = await axios.get('https://api.themoviedb.org/3/trending/movie/week?language=pt-BR', options);
    ...
    // Chamando a função de busca de filmes ao carregar o componente
    fetchMovies();
  }, []); // O segundo argumento vazio significa que essa função será executada apenas uma vez, equivalente a componentDidMount em classes
  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      <section className='flex flex-wrap gap-4 p-4'>
        {filmes && filmes.map(filme => <Card filme={filme} key={filme.id} />)}
      </section>
